...
from v1_no_TCA_multi_source_model import notca_federated_source_and_target_model, digit_five_only_RF_MMD_notca_federated_source_and_target_model
...
from dataset_federated import dataset_federated, digit_five_dataset_federated
import random
import numpy as np
import math
...
if __name__ == '__main__':
    ...
    # weight_IM_list = [0.1, 0.5, 1.0, 2.0]

    for target in target_list:
        # for Ts in [50, 100, 200, 400, 800]:        
        # for Ts in [50]:
        for Ts in [800]:
            # Setup output
            data = time.strftime("%Y%m%d-%H-%M", time.localtime())
            exp = data + 'Average'
            log_file = '{}_additional_Ts_={}_real_two_stage_and_RFMMD_source_init_all_clients_{}_{}.txt'.format(dataset_name, Ts, target, exp)
            ...
            exp_repeated_num = 2 # 5
            pretrain_epoch = 5
            target_self_adapt_epoch = 40

            log("epoches = {}, iter_num = {}".format(epoches, iter_num))
            log("Ts classifier aggragation cycle.")
            log("when epoch < pretrain_epoch={}, only source feature extractor update its parameters. ".format(pretrain_epoch))
            ...
            # log("one stage means that no source pretrain on local. ")
            # log("Twice MMD means that mmd_loss = MMD(X_S, X_T) + MMD(\Sigma_S, \Sigma_T)")
            log("only RF_MMD means that only Sigma_S and Sigma_T participant in MMD calculation.")
            ...
            for currently_i in range(exp_repeated_num):
                ...
                log("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
                log('currently_i = {}, \t Ts = {}'.format(currently_i, Ts))
                ...
                # model setting
                # classifier = notca_federated_source_and_target_model(feature_dim=feature_dim,class_num=class_num).to(device)
                classifier = digit_five_only_RF_MMD_notca_federated_source_and_target_model(feature_dim=feature_dim, n_features=(feature_dim//2), sigma=sigma, kernel='rbf', class_num=class_num).to(device)
                ...
                for epoch in range(epoches):
                    ...
                    for iter_i in tqdm(range(iter_num)):
                        ...
                        temp_source4_Sigma_yi = x_s4.detach()
                        ...
                        mmd_loss_source4 = classifier.mmd_loss(x_s4, temp_target_Sigma_yi)
                        ...
                        # target MMD loss on random subset of source message
                        ...
                        mmd_loss_target4 = classifier.mmd_loss(temp_source4_Sigma_yi, x_t)
                        target_mmd_loss_list = [mmd_loss_target1, mmd_loss_target2, mmd_loss_target3, mmd_loss_target4]
